...
export const GetMatch = () => useQuery({
    retry: 2,
    queryKey: "getMatch",
    queryFn: async (id):Promise<Match> => await ApiClient.get(`/match/${id}`)
        .then(res => {
...
export const CreateMatch = () => useMutation({
    retry: 2,
    mutationKey: "createMatch",
    mutationFn: async (match:Match):Promise<void> => await ApiClient.post("/match/", match)
