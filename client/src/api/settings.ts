...
export const GetTableCount = () =>
  useQuery({
    ...
    queryFn: async (): Promise<number> =>
      await ApiClient.get("/settings/tableCount")
        .then((res) => {
...
export const SetTableCount = () =>
  useMutation({
    retry: 2,
    mutationKey: "setTableCount",
    mutationFn: async (settings: Settings): Promise<void> =>
      await ApiClient.put("/settings/tableCount", settings)
